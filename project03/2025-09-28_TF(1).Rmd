...
def GibbsMotifFinder (seqs, k, seed=42):
    ...
    stability_window = 50  # 连续50次迭代的变化量小于阈值
...
1.  ~~Test function error\
    ~~通过在 `from seq_ops import ...` 语句中加入 `COMP_TRANS`，你确保了在调用 `get_seq` 和 `reverse_complement` 之前，Python 的当前命名空间已经加载了 `COMP_TRANS` 变量。这样，当 `reverse_complement` 执行时，它就能成功找到所需的翻译表，程序也就能正常运行了。~~\
    ~~请在你的 R Markdown 文件中进行这个修改，并重启 Python 会话（或 RStudio 会话）以清除之前的错误状态，然后再次运行代码。
...
Traceback (most recent call last):
  ...
  File "/courses/BINF6250.202610/students/feng.tiang/project_03/seq_ops.py", line 39, in get_seq
    promoter_seq = seq[(start-size-1):(start-1)]
                   ~~~^^^^^^^^^^^^^^^^^^^^^^^^^^
  File "/courses/BINF6250.202610/students/feng.tiang/project_03/seq_ops.py", line 21, in reverse_complement
